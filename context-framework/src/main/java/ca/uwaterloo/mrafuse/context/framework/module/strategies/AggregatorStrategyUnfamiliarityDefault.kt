...
class AggregatorStrategyUnfamiliarityDefault : AggregatorStrategy {
    override fun aggregateData(alpha: Int): Double {
        var sum = 0.0
        var count = 0
        for (eachApi in PluginManager.instance!!.pluginListReadOnly) {
            if (PLUGIN_TYPE.unfamiliarity == eachApi.pluginType) {
                sum += eachApi.statusDataUnfamiliarity!!.unfamiliarity
                count++
            }
        }
        ...
        return if (estimate > 0) {
            ...
            alpha.toDouble().pow(exponent) / alpha.toDouble()
        } else 0.0
    }
}