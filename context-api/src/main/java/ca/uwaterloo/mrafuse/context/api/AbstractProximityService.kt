 ...
 */
abstract class AbstractProximityService : AbstractPluginService() {
    private val mBinder = ProximityPluginServiceBinder()
    protected fun publishRiskUpdate(statusDataProximity: StatusDataProximity?) {
        val bundle = createProximityDataBundle(statusDataProximity)
        publishDataUpdate(bundle)
    }

    override val pluginType: PLUGIN_TYPE
        get() = PLUGIN_TYPE.proximity

    private fun createProximityDataBundle(statusDataProximity: StatusDataProximity?): Bundle {
        val bundle = Bundle()
