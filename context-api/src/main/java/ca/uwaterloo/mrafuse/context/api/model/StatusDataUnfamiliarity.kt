...
class StatusDataUnfamiliarity : Parcelable {
    enum class Status {
    ...
    fun info(info: String?): StatusDataUnfamiliarity {
        this.info = info
        return this
    }

    fun status(status: Status?): StatusDataUnfamiliarity {
        this.status = status
        return this
    }

    fun unfamiliarity(unfamiliarity: Int): StatusDataUnfamiliarity {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val that = other as StatusDataUnfamiliarity
        if (java.lang.Double.compare(that.unfamiliarity.toDouble(), unfamiliarity.toDouble()) != 0) return false
        if (status != that.status) return false
        return if (info != null) info == that.info else that.info == null
    }

    override fun hashCode(): Int {
        var result: Int = if (status != null) status.hashCode() else 0
        val temp: Long = java.lang.Double.doubleToLongBits(unfamiliarity.toDouble())
    ...
    companion object {
         ...
         */
        @JvmField
        val CREATOR: Parcelable.Creator<StatusDataUnfamiliarity?> = object : Parcelable.Creator<StatusDataUnfamiliarity?> {
            override fun createFromParcel(`in`: Parcel): StatusDataUnfamiliarity? {
                return StatusDataUnfamiliarity(`in`)
            }

            override fun newArray(size: Int): Array<StatusDataUnfamiliarity?> {
